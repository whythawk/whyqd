...
Once you have reviewed your data, and worked through the script you need to produce, they can be assigned::
    ...
    >>> method.add_actions(actions=schema_scripts, uid=source_data.uuid.hex, sheet_name = "Data")
...
Wide format is not exactly helpful, so we'll modify this::
    ...
    >>> method.add_actions(actions=schema_script, uid=source_data.uuid.hex, sheet_name = "Data")
    >>> df = method.transform(source_data)
...
from whyqd.metamodel.models import (
    DataSourceModel,
    ActionScriptModel,
    MethodModel,
    CategoryActionModel,
...
from whyqd.transform.parsers import CoreScript, WranglingScript, MethodScript, ParserScript
...
class Method:
    ...
    def __init__(
        ...
        self.mthdprsr = MethodScript(schema=self._schema)
        self.wrangle = WranglingScript()
        self.parser = ParserScript()
        # Set working directory
        self.directory = self.core.check_path(directory=directory)
    ...
    def add_data(
        ...
        for data in source:
            ...
            if self.core.check_uri(source=data.path):
                # File at remote URI
                ...
                try:
                    ...
                    pass
            self.core.rename_file(source=local_source, newname=data.source)
            if not get_row_count:
                df_sample = self.wrangle.get_dataframe(
                    source=self.directory / data.source,
                    filetype=data.mime,
            ...
            for k in df_sample.keys():
                df_columns = self.wrangle.get_dataframe_columns(df=df_sample[k])
                data_k = data.copy()
    ...
    def reorder_data(self, order: List[Union[UUID, Tuple[UUID, str]]]) -> None:
        ...
        """
        if self._method.input_data:
            self._method.input_data = self.mthdprsr.reorder_models(model_list=self._method.input_data, order=order)
    ...
    def add_actions(self, actions: Union[str, List[str]], uid: UUID, sheet_name: Optional[str] = None) -> None:
        ...
        for a in actions:
            ...
            source_data = self.mthdprsr.get_source_data(method=self._method, uid=uid, sheet_name=sheet_name)
            params = self.mthdprsr.parse_action_script(source_data=source_data, script=a)
            ...
            if isinstance(params["action"], MorphActionModel):
                ...
                if pre_df.empty:
                    ...
                    pre_df = self.mthdprsr.transform_df_from_source(df=pre_df, source_data=source_data, **params)
                    # And update the columns
                    source_data.columns = self.wrangle.get_dataframe_columns(df=pre_df)
    ...
    def update_action(self, uid: UUID, action_uid: UUID, action: str, sheet_name: Optional[str] = None) -> None:
        ...
        self.mthdprsr.parse_action_script(source_data=source_data, script=new_action)
        source_data = self.mthdprsr.update_actions(action_list=source_data.actions, uid=action_uid, new_action=new_action)
        self._rebuild_actions(source_data)
    ...
    def reorder_actions(self, uid: UUID, order: List[UUID], sheet_name: Optional[str] = None) -> None:
        ...
        """
        source_data = self.mthdprsr.get_source_data(self._method, uid=uid, sheet_name=sheet_name)
        source_data = self.mthdprsr.reorder_models(model_list=source_data.actions, order=order)
        self._rebuild_actions(source_data)
    ...
    def merge(self, script: str) -> None:
        ...
        merge_list = self.mthdprsr.parse_merge(script=actions[0], input_data=self._method.input_data)
        # Perform the merge
        ...
        working_data.columns = self.wrangle.get_dataframe_columns(df=df)
        ...
        df = self.wrangle.get_dataframe_from_datasource(data=working_data)
        working_data.checksum = self.core.get_data_checksum(df=df)
    ...
    def transform(self, data: DataSourceModel) -> pd.DataFrame:
        ...
        if self.parser.get_anchor_action(script=data_actions[0].script).name == "MERGE":
        ...
        data.columns = self.wrangle.get_dataframe_columns(df=df)
        # 1. Parse all category assignment scripts
        category_assignments = []
        for script in data.actions:
            first_action = self.parser.get_anchor_action(script=script.script)
            if isinstance(first_action, CategoryActionModel) and first_action.name in [
                "ASSIGN_CATEGORY_UNIQUES",
                "ASSIGN_CATEGORY_BOOLEANS",
            ]:
                category_assignments.append(self.mthdprsr.parse_action_script(source_data=data, script=script))
        parsed = []
        ...
        for script in data_actions:
            ...
            if isinstance(first_action, CategoryActionModel):
                continue
            p = self.mthdprsr.parse_action_script(source_data=data, script=script)
            if isinstance(p["action"], MorphActionModel):
                ...
                pre_parsed.append(p)
                data.columns = self.wrangle.get_dataframe_columns(df=pre_df)
                for params in pre_parsed:
                    pre_df = self.mthdprsr.transform_df_from_source(df=pre_df, source_data=data, **params)
                    data.columns = self.wrangle.get_dataframe_columns(df=pre_df)
        ...
        for params in parsed:
            df = self.mthdprsr.transform_df_from_source(df=df, source_data=data, **params)
            # And update the columns
            data.columns = self.wrangle.get_dataframe_columns(df=df)
        return df
    ...
    def build(self) -> None:
        ...
        restructured_data.columns = self.wrangle.get_dataframe_columns(df=df_restructured[header_order])
        restructured_data.preserve = [c for c in restructured_data.columns if c.type_field == "string"]
        # Load file again to calculate checksum
        df_restructured = self.wrangle.get_dataframe_from_datasource(data=restructured_data)
        restructured_data.checksum = self.core.get_data_checksum(df=df_restructured[header_order])
    ...
    def validate(self) -> bool:
        ...
        temporary_data.columns = self.wrangle.get_dataframe_columns(df=df_restructured)
        temporary_data.preserve = [c for c in temporary_data.columns if c.type_field == "string"]
        # Load file again to calculate checksum
        df_restructured = self.wrangle.get_dataframe_from_datasource(data=temporary_data)
        # Validate RESTRUCTURED DATA checksums
        df_checksum = self.core.get_data_checksum(df=df_restructured)
        # Delete the temporary file. Before crashing.
        self.core.delete_file(source=temporary_path)
    ...
    def save(
        ...
        for data in self._method.input_data:
            ...
            data.columns = self.wrangle.get_dataframe_columns(df=df)
        if self._method.working_data:
            df = self.wrangle.get_dataframe_from_datasource(data=self._method.working_data)
            self._method.working_data.columns = self.wrangle.get_dataframe_columns(df=df)
        if self._method.restructured_data:
            df = self.wrangle.get_dataframe_from_datasource(data=self._method.restructured_data)
            self._method.restructured_data.columns = self.wrangle.get_dataframe_columns(df=df)
    ...
    #     path = data.path
    #     try:
    #         self.core.check_source(source=path)
    ...
    def _rebuild_actions(self, data: DataSourceModel) -> None:
        ...
        data.columns = self.wrangle.get_dataframe_columns(df=df)
        data.actions = []
        data.actions = self.add_actions(actions=actions, uid=data.uuid.hex, sheet_name=data.sheet_name)
    ...
    def _merge_dataframes(self, merge_list: List[DataSourceModel]) -> pd.DataFrame:
        ...
        for input_data in merge_list:
            ...
            if input_data.actions:
                ...
                df = self.wrangle.get_dataframe_from_datasource(data=input_data)
            if df_base.empty:
                df_base = df.copy()
        ...
        df_base.columns = self.wrangle.deduplicate_columns(df=df_base, schema=self._schema)
        return df_base
    ...
    def _restructure_dataframes(self) -> pd.DataFrame:
        ...
        if len(self._method.input_data) == 1 and not isinstance(self._method.working_data, DataSourceModel):
            ...
            merge_list = self.mthdprsr.parse_merge(script=merge_script, input_data=self._method.input_data)
            ...
            temporary_data = DataSourceModel(**{"path": str(temporary_path)})
            temporary_data.columns = self.wrangle.get_dataframe_columns(df=df_working)
            temporary_data.preserve = [c for c in temporary_data.columns if c.type_field == "string"]
            # Load file again to calculate checksum
            df_working = self.wrangle.get_dataframe_from_datasource(data=temporary_data)
            df_checksum = self.core.get_data_checksum(df=df_working)
