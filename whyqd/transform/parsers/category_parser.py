...
if TYPE_CHECKING:
    from whyqd.metamodel.models import ColumnModel, FieldModel, CategoryActionModel, DataSourceModel
...
class CategoryScript:
    ...
    * For `ASSIGN_CATEGORY_BOOLEANS` values from the ColumnModel are treated as boolean `True` or `False`, defined by `::bool`.
    ...
    def parse(
        ...
        source = self.parser.get_field_model(name=source, fields=self.source_columns)
        if not source:
        ...
        if parsed.get("source_category"):
            ...
            assigned = self.get_assigned_uniques(text=parsed["source_category"])
        ...
        destination = self.schema.get_field(name=destination)
