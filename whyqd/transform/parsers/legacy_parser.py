...
from whyqd.transform.parsers import CoreScript, ParserScript
...
class LegacyScript:
    ...
    def parse_legacy_method(self, *, version: str, schema: Type[Schema], source_path: str) -> List[str]:
        ...
        list of str
        """
        self.core.check_source(source=source_path)
        source_path = Path(source_path)
        if version in ["0", "1"]:
            return self.generate_scripts_from_method_v0(schema=schema, source_path=source_path)
        else:
    ...
    def generate_scripts_from_method_v0(self, *, schema: Type[Schema], source_path: Path) -> List[str]:
        ...
        list of str
        """
        method = self.core.load_json(source=source_path)
        # SCHEMA SCRIPTS
        schema_scripts = []
        for fld in method["fields"]:
            dstn = schema.get_field(name=fld["name"])
            actn = self.parser.get_action_model(action=fld["structure"][0]["name"])
            src_strctr = ""
            cat_scrpt = []
            if actn.structure and actn.name != "NEW":
                strct = []
                for src_trms in self.core.chunks(lst=fld["structure"][1:], n=len(actn.structure)):
