...
def save_file(data: json, source: str, overwrite: Optional[bool] = False) -> bool:
    """Save json to file. Return `False` if file `source` already exists and not `overwrite`.
...
def get_dataframe(source, foreign_key=None, filetype=None, **kwargs):
    """
    ...
    if foreign_key and "dtype" not in kwargs:
        kwargs["dtype"] = {foreign_key: object}
    # Get file delimiter
...
def get_dataframe_summary(source, rows=short_rows, foreign_key=None, **kwargs):
