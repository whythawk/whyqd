...
from whyqd.transform.parsers import ActionScript, ParserScript, CategoryScript, MorphScript, FilterScript
from whyqd.metamodel.models import SchemaActionModel, MorphActionModel, CategoryActionModel, FilterActionModel, ColumnModel
...
if TYPE_CHECKING:
    ...
    from whyqd.metamodel.models import DataSourceModel, ActionScriptModel, MethodModel
...
class MethodScript:
    ...
    def get_source_data(self, *, method: MethodModel, uid: UUID, sheet_name: Optional[str] = None) -> DataSourceModel:
        ...
        if method.working_data and method.working_data.uuid == UUID(uid):
            ...
            source = self.get_input_data(input_data=method.input_data, uid=uid, sheet_name=sheet_name)
        if not source:
    ...
    def parse_action_script(
        ...
        first_action = self.parser.get_anchor_action(script=scrpt)
        ...
        if isinstance(first_action, SchemaActionModel):
            return ActionScript(source_columns=source_data.columns, schema=self.schema).parse(script=scrpt)
        if isinstance(first_action, MorphActionModel):
            return MorphScript(data=source_data, schema=self.schema).parse(script=scrpt)
        if isinstance(first_action, CategoryActionModel):
            return CategoryScript(source_data=source_data, schema=self.schema).parse(script=scrpt)
        if isinstance(first_action, FilterActionModel):
            return FilterScript(data=source_data, schema=self.schema).parse(script=scrpt)
    ...
    def transform_df_from_source(self, *, df: pd.DataFrame, source_data: DataSourceModel, **params) -> pd.DataFrame:
        ...
        if isinstance(params["action"], SchemaActionModel):
            return ActionScript(source_columns=source_data.columns, schema=self.schema).transform(df=df, **params)
        if isinstance(params["action"], MorphActionModel):
            return MorphScript(data=source_data, schema=self.schema, df=df).transform(**params)
        if isinstance(params["action"], FilterActionModel):
            return FilterScript(data=source_data, schema=self.schema).transform(df=df, **params)
    ...
    def parse_merge(self, *, script: ActionScriptModel, input_data: List[DataSourceModel]) -> List[DataSourceModel]:
        ...
        """
        action_model = self.parser.get_anchor_action(script=script.script)
        ...
        MERGE = self.parser.get_action_class(actn=action_model)
        merge_terms = MERGE().parse(script=script.script)
        merge_list = []
        # Check if the source data exist
        for key, uid, sheet_name in merge_terms:
            data = self.get_input_data(input_data=input_data, uid=uid, sheet_name=sheet_name)
            if not data:
