...
from whyqd.metamodel.models import MethodModel, DataSourceModel
from whyqd.metamodel.schema import Schema
from whyqd.metamodel.method import Method
from whyqd.transform.parsers import CoreScript, WranglingScript
...
class Validate:
    ...
    def set(self, *, source: Optional[str] = None, method: Optional[MethodModel] = None) -> None:
        ...
        self._method = None
        if source:
            self._method = self.core.load_json(source=source)
        elif method:
            self._method = method
        if not self._method:
            raise ValueError(
    ...
    def import_input_data(self, *, path: Union[str, List[str]]) -> None:
        ...
        if isinstance(path, str):
            path = [path]
        for p in path:
            self.core.check_source(source=p)
            checksum = self.core.get_checksum(source=p)
    ...
    def import_restructured_data(self, *, path: str) -> None:
        ...
        df = self.wrangle.get_dataframe_from_datasource(data=DataSourceModel(**restructured_data))
        ...
        if checksum != self._method.restructured_data["checksum"]:
            ...
            checksum = self.core.get_data_checksum(df=df[header_order])
            if checksum != self._method.restructured_data["checksum"]:
                raise ValueError(
    ...
    def validate(self) -> bool:
        ...
        if merge_script:
            ...
            if working_scripts:
                method.add_actions(actions=working_scripts, uid=working_data.uuid.hex)
        # method.build()
