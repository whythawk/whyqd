...
refactored from https://raw.githubusercontent.com/pandas-dev/pandas/v1.5.1/pandas/core/util/hashing.py
"""
...
def hash_pandas_object(
    ...
    if isinstance(obj, pd.MultiIndex):
        return pd.Series(hash_tuples(obj, encoding, hash_key), dtype="uint64", copy=False)

    elif isinstance(obj, pd.Index):
        h = hash_array(obj.values, encoding, hash_key, categorize).astype("uint64", copy=False)
        ser = pd.Series(h, index=obj, dtype="uint64", copy=False)
        ...
        ser = pd.Series(h, index=obj.index, dtype="uint64", copy=False)

    elif isinstance(obj, pd.DataFrame):
        hashes = (hash_array(series.values, encoding, hash_key, categorize) for _, series in obj.items())
...
def hash_tuples(
    vals: pd.MultiIndex | Iterable[tuple[Hashable, ...]],
    encoding: str = "utf8",
    ...
    if not isinstance(vals, pd.MultiIndex):
        mi = pd.MultiIndex.from_tuples(vals)
    else:
        mi = vals

    # create a list-of-Categoricals
    cat_vals = [
        pd.Categorical(mi.codes[level], mi.levels[level], ordered=False, fastpath=True) for level in range(mi.nlevels)
    ]
...
def _hash_categorical(cat: pd.Categorical, encoding: str, hash_key: str) -> npt.NDArray[np.uint64]:
    """
