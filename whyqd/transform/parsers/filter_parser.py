...
if TYPE_CHECKING:
    from whyqd.metamodel.models import ColumnModel, FieldModel, DataSourceModel
    from whyqd.metamodel.schema import Schema
    from whyqd.transform.base import BaseFilterAction
...
class FilterScript:
    ...
    def parse(self, *, script: str) -> Dict[str, Union[FilterActionModel, Optional[List[ColumnModel]], Optional[date]]]:
        ...
        parsed = self.parser.get_action_class(actn=action_model)().parse(script=script)
        # Filter column
        filter_column = self.parser.get_literal(text=parsed["filter"])
        filter_column = self.parser.get_field_from_script(name=filter_column, fields=self.source_columns, schema=self.schema)
        # Column
        column = None
        if parsed.get("column"):
            column = self.parser.get_literal(text=parsed["column"])
            column = self.parser.get_field_from_script(name=column, fields=self.source_columns, schema=self.schema)
        # Date term
        date_term = None
        if parsed.get("date"):
            date_term = self.parser.get_literal(text=parsed["date"])
    ...
    def transform(
        ...
        *,
        ...
        return action.transform(df=df, filter_column=filter_column, date_term=date_term, column=column)
