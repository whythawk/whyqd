...
class Field:
    ...
    def __init__(self, name, field_type, **kwargs):
        ...
        self.field_settings = {"name": self._name, "type": self._type, "constraints": {}}
    ...
    def set_constraint(self, constraint, value):
        ...
        if constraint in ["minimum", "maximum"]:
            if (
                self.default_constraints[constraint]["type"]
                in [
                    "string",
                    "integer",
                    "number",
                ]
                and constraint_type not in ["integer", "number"]
            ):
                e = "Constraint `{}` should be an integer or number.".format(constraint)
                raise ValueError(e)
            if self.default_constraints[constraint]["type"] in ["string", "integer"] and constraint_type == "number":
                e = "Constraint `{}` should be an integer".format(constraint)
                raise ValueError(e)
        if constraint == "category":
            # Category constraints are very specific
            if not (isinstance(value, list) and isinstance(value[0], dict) and all(["name" in v for v in value])):
        ...
        if constraint == "filter":
            ...
            if not value.get("modifiers"):
                ...
                raise KeyError(e)
            if "field" in value and not (isinstance(value["field"], str) or isinstance(value["field"], bool)):
