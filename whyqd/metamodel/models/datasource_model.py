...
from whyqd.metamodel.dtypes import MimeType
from whyqd.metamodel.models import CitationModel, ColumnModel, ActionScriptModel
...
class DataSourceModel(BaseModel):
    ...
    def generate_checksum(cls, v, values, **kwargs):
        check = CoreScript().get_checksum(source=values["path"])
        if v and v != check:
            ...
            check = CoreScript().get_data_checksum(df=df)
            if v and v != check:
                raise ValueError(
