...
from whyqd.metamodel.models import VersionModel, ConstraintsModel, FieldModel, SchemaModel, CategoryModel
...
class Schema:
    ...
    def __init__(self, *, source: Optional[Union[str, SchemaModel]] = None) -> None:
        ...
        if source:
            if isinstance(source, str):
                self._schema = SchemaModel(**self.core.load_json(source=source))
    ...
    def add_field(self, *, field: FieldModel) -> None:
        ...
        if self.get_field(name=new_field.name):
    ...
    def update_field(self, *, field: FieldModel) -> None:
        ...
        new_field = FieldModel(**field)
        old_field = self.get_field(name=new_field.name)
        if not old_field:
        ...
        self.add_field(field=old_field.dict(by_alias=True, exclude_defaults=True, exclude_none=True))
    ...
    def set_field_constraints(self, *, name: str, constraints: Union[ConstraintsModel, None]) -> None:
        ...
        """
        old_constraints = self.get_field_constraints(name=name)
        if not constraints:
        ...
        self.get_field(name=name).constraints = old_constraints
    ...
    def get_field_category(self, *, field: str, category: Union[bool, str]) -> Union[CategoryModel, None]:
        ...
        """
        if self._schema:
            field_categories = self.get_field_constraints(name=field)
            if not field_categories and isinstance(category, bool):
                ...
                fld = self.get_field(name=field)
                fld.constraints = ConstraintsModel(**{"default": true, "enum": [true, false]})
            field_categories = self.get_field_constraints(name=field).category
    ...
    def save(
        ...
        *,
        ...
        return self.core.save_file(data=self.get_json(hide_uuid=hide_uuid), source=directory + filename)
