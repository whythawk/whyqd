...
from whyqd.transform.action import default_actions, actions
...
if TYPE_CHECKING:
    ...
    from whyqd.transform.base import BaseSchemaAction, BaseMorphAction, BaseCategoryAction
...
class ParserScript:
    ...
    def get_field_from_script(
        ...
        # Is it in the schema?
        field = schema.get_field(name=name)
        if not field:
            # Is it a ColumnModel?
            field = self.get_field_model(name=name, fields=fields)
        if not field:
            raise ValueError(
