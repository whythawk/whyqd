...
from whyqd.transform.action import default_actions
from whyqd.metamodel.models import ConstraintsModel, FieldModel

if TYPE_CHECKING:
    from whyqd.metamodel.models import ColumnModel, SchemaActionModel, ModifierModel, CategoryActionModel, CategoryModel
...
class ActionScript:
    ...
    def parse(self, *, script: str) -> Dict[str, Union[SchemaActionModel, List[ColumnModel], dict]]:
        ...
        normalised_script = self._get_normalised_script(script=script)
        # Process first part
        root = self.parser.get_split_terms(script=normalised_script, by="<")
        ...
        action = self.parser.get_action_model(action=split_terms[0])
        if not action:
        ...
        if len(split_terms) == 2:
            ...
            destination = self.parser.get_field_from_script(name=destination, fields=self.source_columns, schema=self.schema)
        ...
        for i, stack in list(self.parser.generate_contents(text=source)):
            ...
            if last_i == i:
                ...
                parsed.append((stack, {"action": None, "source": self.parser.get_listed_literal(text=stack)}, uuid4().hex))
            else:
        ...
        source = self._get_recovered_source_columns(source=source)[0]
        parsed = {"action": action, "destination": destination, "source": source}
        self._check_validates(source=parsed)
        return parsed
    ...
    def transform(
        ...
        *,
        ...
        if action.name == "CATEGORISE" and assigned:
            return action.transform(df=df, destination=destination, source=flattened_source, assigned=assigned)
        else:
    ...
    def _get_normalised_script(self, *, script: str) -> str:
        ...
        str
        """
        normalised_script = self.parser.get_normalised_script(script=script, fields=self.source_columns)
        self.modifier_names = set()
        self.source_modifiers = {}
    ...
    def _get_recovered_source_columns(self, *, source: Union(list, dict)) -> list:
        ...
        for term in source:
            ...
            if isinstance(term, str) and term in self.modifier_names:
                ...
                recovered = self.parser.get_field_from_script(name=term, fields=self.source_columns, schema=self.schema)
            elif isinstance(term, list):
                recovered = self._get_recovered_source_columns(source=term)
                ...
                recovered = {
                    ...
                    "source": self._get_recovered_source_columns(source=term.get("source")),
                }
            if not recovered:
    ...
    def _check_validates(self, *, source: Union(list, dict)) -> bool:
        ...
        for source_strut in self.core.chunks(lst=source["source"], n=len(structure)):
